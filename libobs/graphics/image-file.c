...
{
	return image->gif_info->width * image->gif_info->height * 4 *
	       image->gif_info->frame_count;
}
			      ...
			      enum gs_image_alpha_mode alpha_mode)
{
	...
	nsgif_create(&image->bitmap_callbacks, NSGIF_BITMAP_FMT_R8G8B8A8,
		     &image->gif);
	...
	if (image->gif_info->width > 4096 || image->gif_info->height > 4096) {
		blog(LOG_WARNING, "Bad texture dimensions (%dx%d) in '%s'",
		     image->gif_info->width, image->gif_info->height, path);
		goto fail;
	}

	max_size = (uint64_t)image->gif_info->width *
		   (uint64_t)image->gif_info->height *
	...
	image->is_animated_gif =
		(image->gif_info->frame_count > 1 && result >= 0);
	if (image->is_animated_gif) {
		nsgif_frame_decode(image->gif, 0, &bitmap);

		image->animation_frame_cache = alloc_mem(
			image, mem_usage,
			image->gif_info->frame_count * sizeof(uint8_t *));
		image->animation_frame_data = alloc_mem(
			image, mem_usage, get_full_decoded_gif_size(image));

		for (unsigned int i = 0; i < image->gif_info->frame_count;
		     i++) {
			if (nsgif_frame_decode(image->gif, i, &bitmap) !=
			    NSGIF_OK)
				blog(LOG_WARNING,
...
static inline uint64_t get_time(gs_image_file_t *image, int i)
{
	const nsgif_frame_info_t *gif_frame_info =
		nsgif_get_frame_info(image->gif, i);
			     ...
			     enum gs_image_alpha_mode alpha_mode)
{
	if (!image->animation_frame_cache[new_frame]) {
		...
		if (nsgif_frame_decode(image->gif, new_frame, &bitmap) ==
		    NSGIF_OK) {
			const size_t area = (size_t)image->gif_info->width *
					    image->gif_info->height;
